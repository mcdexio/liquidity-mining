...
def create_watcher():
    ...
    uniswap_mcb_usdc_share_token_tracer = ERC20Tracer(config.UNISWAP_MCB_USDC_SHARE_TOKEN_ADDRESS, web3, mining_round.end_block_number)
    ...
    syncers = [eth_perp_share_token_tracer, eth_perp_position_tracer, uniswap_mcb_share_token_tracer, uniswap_mcb_usdc_share_token_tracer,
...
def main():
    ...
    if args.rollback is not None:
        ...
        if args.extradata == 'uniswap_mcb_share':
            # sync uniswap mcb/usdc share event
            end_block_number = 10932499  #qin end number
            watcher_id = 104
            share_tokens = [config.UNISWAP_MCB_USDC_SHARE_TOKEN_ADDRESS]
            utils.sync_extradata(args.extradata, end_block_number, watcher_id, share_tokens)
    else:
        serv()
